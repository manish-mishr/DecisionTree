...
def Tree(parent,train_data, attr_list,dict_class,algo,classT=None):
	global Total_Length,global_count
	...
	try:
		nodeCost = log(node.NodeCount,2)
	except:
		nodeCost = 0

	try:
		leafCost = log(node.LeafCount,2)
	except:
		leafCost = 0

	(perror,total) = error_count(train_data)
	try:
		perror = perror * log(Total_Length,2) + nodeCost + leafCost
	except:
		perror = 0.5

	IncreaseCost = log(node.NodeCount+1,2)
	IncreaseCost += log(node.LeafCount+2,2)
	curr_error = log(Total_Length,2)*(lerror+rerror) + IncreaseCost
